...
def posts_create_data(soup):
    ...
    date_size = len(df_create_dates2)
    df_next = df_create_dates2['date'].ix[1:date_size - 1].reset_index()
    df_next.columns = ['index', 'next_date']
    df_now_next = DataFrame(df_create_dates2['date']).join(DataFrame(df_next['next_date']))
